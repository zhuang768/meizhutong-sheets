...
- 送件後立刻跑確定性規則查核並回覆 App。GPT 看圖請用試算表選單補跑，避免收件網址等模型而逾時。
...
4. 同一請求內先跑規則查核（只填「AI 查核」欄）並立刻回覆成功或失敗。GPT 看圖改走選單「GPT 核對」，不擋青年端送出。人工審查決定維持「待審」。
...
2. **GPT 看圖**（有 `OPENAI_API_KEY` 才跑；預設模型 `gpt-4o`）  
   請用選單「GPT 核對身分證／發票／切結書」補跑，不放在送件 HTTP 裡，以免 App 等模型等到逾時。比對身分證姓名／字號／生日／戶籍、收據購買人／工具／公司／日期／金額、切結書是否有手寫簽名且姓名相符。看不清楚就標補件，不假裝已辨識。
